...
```
```{r}
ebV <- cluster_edge_betweenness(AdyacenciaV, directed = TRUE)
table(membership(ebV))
...
plot(ebV, AdyacenciaV)#separa y colorea los clusters
...
```{r}
Edad <- read.csv("edad.xlsx - Hoja1.csv")
## Establecemos los nombres de las columnas en la fila 1
rownames(Edad)<-Edad[,1]
#Eliminamos la fila 1 de la base de datos que contiene nombres
Edad <- Edad[,-1]
#Visualizamos nuestra base de datos
Edad
...
```{r}
#Convertimos nuestra base de datos en una matriz
EdadMM <- as.matrix(Edad)
#Vamos a hacer una matriz de correlación para tener valores entre 0 y 1 que nos indiquen 
#la correlación entre infividuos
EdadCor <- cor(t(EdadMM))
#ponemos los valores de la diagonal en 0
diag(EdadCor)<-0
...
diag(EdadAd1)<-0
#Indicamos que los valores mayores a 0.75 se les ponga un 1
Ecerouno <- ifelse(EdadAd1>0.75,1,0)
#Convertimos a matriz
Ecerouno <- as.matrix(Ecerouno)

#Cargamos la libreria de igraph
library(igraph)

#Convertimos nuestra matriz en una matriz de Adyacencia
AdyacenciaE <- graph_from_adjacency_matrix(Ecerouno)


V(AdyacenciaE)$size<-degree(AdyacenciaV,mode="in")*3  #Hacemos el tamaño de los nodos por el valor del degree de entrada
V(AdyacenciaE)$color="#96F7D2" #les ponemos colorcito
V(AdyacenciaE)$shape="sphere" #establecemos forma
E(AdyacenciaE)$color="white" #damos color a las conexiones
...
plot(AdyacenciaE,edge.arrow.size=0.15,edge.curved=0.3,vertex.label.font=15, 
     vertex.label.color="#F1F7E7",vertex.label.cex=0.7)
```
```{r}
ebE <- cluster_edge_betweenness(AdyacenciaE, directed = TRUE)
ebE
table(membership(ebE))

#Nos indica la presencia de 4 clusters
#Grupo 1 +41 años (6 individuos)
#Grupo 2 21 a 30 años (38 individuos)
#Grupo 3 31 a 40 años (4 individuos)
#Grupo 4 15 a 20 años (8 individuos)
```
```{r, echo=FALSE}
#Hacemos el fondo de los plots a color negro
par(bg="black")
...
plot(ebE, AdyacenciaE)#separa y colorea los clusters
...
resumenD <- summarySE (datos, #base de datos
                      measurevar = "Dosis", 
                      groupvars = "Sector") 
resumenD #imprime el resumen con los datos con, sd, se y ci
...
resumenI #imprime el resumen con los datos con, sd, se y ci
...
resumenE <- summarySE (datos, #base de datos
                      measurevar = "Edad", 
                      groupvars = "InfVacunacion") 
resumenE #imprime el resumen con los datos con, sd, se y ci
...
resumenEd <- summarySE (datos, #base de datos
                      measurevar = "Edad", 
                      groupvars = "Dosis") 
resumenEd #imprime el resumen con los datos con, sd, se y ci
...
library (ggplot2) 
graficaEd <- ggplot(resumenEd, aes(x = Dosis, y = Edad, fill = Dosis)) + #objeto con los datos y su variable, rellenar color por dietas
    geom_bar(position = position_dodge (), stat = "identity") + #tipo de grafica de barras
    geom_errorbar(aes(ymin = Edad - ci, ymax = Edad + ci), #intervalo de confianza
                  width =.1, 
                  position = position_dodge(.9)) #cargar datos de la grafica
...
#El sector dónde se desarrollan tiene efectos en el número de dosis que se pusieron
pruebaEd <- aov (Edad ~ Dosis , datos)
pruebaEd

#Imprimimos el resumen de nuestra anova
summary (pruebaEd)

#Valor de P de 0.19 se rechza la hipotesis
```